...
def test_winning_color_returns_first_qualifying_color_not_last():
    """HARDENING: winning_color() must break at the first color meeting vps_to_win, not scan
    every color and return the LAST match. Harmless in a real single-mover game (only one
    color can cross the threshold at the moment this is checked), but a latent correctness
    risk for batched tree-search termination checks over hypothetical/simulated states where
    more than one color could satisfy the condition simultaneously."""
    players = [
        RandomPlayer(Color.RED),
        RandomPlayer(Color.BLUE),
        RandomPlayer(Color.WHITE),
    ]
    game = Game(players, vps_to_win=10)

    for color in (Color.RED, Color.BLUE, Color.WHITE):
        key = player_key(game.state, color)
        game.state.player_state[f"{key}_ACTUAL_VICTORY_POINTS"] = 10

    # Seating order (game.state.colors) may be shuffled at Game construction, so the "first"
    # qualifying color is whichever one is first in THAT order, not necessarily Color.RED.
    assert game.winning_color() == game.state.colors[0]
