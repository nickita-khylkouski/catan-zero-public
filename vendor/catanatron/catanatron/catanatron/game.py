...
class Game:
    ...
    def winning_color(self) -> Union[Color, None]:
        ...
        """
        # HARDENING: break at the first qualifying color instead of scanning every color and
        # keeping the LAST match. Normally harmless (only one player crosses the threshold at
        # the moment this is checked in a real game), but a latent correctness risk for batched
        # tree-search termination checks over hypothetical/simulated states where more than one
        # color could satisfy the condition simultaneously.
