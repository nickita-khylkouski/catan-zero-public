...
class AlphaBetaPlayer(Player):
    ...
    def __init__(
        ...
        # FIX A4: `prunning` (list_prunned_actions) collapses MOVE_ROBBER decisions down to a
        # single "most impactful" tile (prune_robber_actions), so a caller that wants every
        # ROOT child scored (e.g. distilling AB search into soft policy/value targets) sees
        # soft_score_legal_coverage collapse to 1/18 at robber decisions. When True, the ROOT
        # ply (depth == self.depth, i.e. the outermost alphabeta call only) uses the full
        # unpruned action set; deeper plies still use `prunning` for search speed.
        self.full_width_root = bool(full_width_root)

    def value_function(self, game, p0_color):
        raise NotImplementedError

    def get_actions(self, game, full_width=False):
        if self.prunning and not full_width:
    ...
    def alphabeta(self, game, depth, alpha, beta, deadline, node):
        ...
        is_root_ply = depth == self.depth
        actions = self.get_actions(
            game, full_width=is_root_ply and self.full_width_root
        )  # list of actions.
