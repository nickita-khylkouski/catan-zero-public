...
def execute_spectrum(game: Game, action: Action):
    ...
    if action.action_type in DETERMINISTIC_ACTIONS:
        ...
        for card in set(current_deck):
            ...
            # FIX A2: without an action_record carrying the enumerated card as its `.result`,
            # apply_buy_development_card (apply_action.py) draws a FRESH random card from the
            # deck instead of the one this branch is supposed to be evaluating — every
            # enumerated outcome collapsed onto the same random draw. Passing the record here
            # makes apply_action honor `card` (see apply_action.py's `action_record is None`
            # branch vs. `card = action_record.result`).
        ...
        for roll in range(2, 13):
            ...
            # FIX A2: same issue as BUY_DEVELOPMENT_CARD above — apply_roll (apply_action.py)
            # rolls FRESH random dice unless given an action_record whose `.result` is the
            # enumerated (d1, d2) outcome, so every one of the 11 "enumerated" dice outcomes
            # was actually resolved with independent fresh randomness (verified live: 10/11
            # mismatched the requested roll before this fix).
            option_action_record = ActionRecord(action=option_action, result=outcome)
            option_game = game.copy()
            option_game.execute(
                option_action,
                validate_action=False,
