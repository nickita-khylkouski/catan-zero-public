...
- Repo: **`github.com/nickita-khylkouski/catan-zero-public`** (**PUBLIC**, no
  ...
  **`v1.4-native-mcts`**, created only at checksum commit B of the two-commit
  transaction below. All earlier tags predate the `catanatron_rs 0.1.7` native
  search API and must not provision or resume native-MCTS lanes.
- Env target: **Python 3.11.15**, **torch cu128** (all H100 + B200), **catanatron_rs 0.1.7 cp311**.
...
1. Create clean **commit A** containing every release source, builder, test, and
   ...
   `catanatron_rs-0.1.7-build-receipt.json`. Require byte-identical wheel
...
refresh are the `catanatron_rs-0.1.7` CPython 3.11 wheel asset, its build
...
- `native/catanatron-rs` v0.1.7 is now the canonical wheel source and builds `catanatron_rs-0.1.7-cp311-…manylinux_2_34`; `native/gumbel_mcts_rs` is its linked native-search dependency. `native/catanatron-rs/WHEEL_SHA256SUMS` seals the exact release asset and the installer rejects any byte mismatch. The build receipt seals the source commit/tree, builder and lockfiles, exact toolchain/environment, and wheel digest. Fleet deployment must be uniform 0.1.7 with `sigma_reference_visits`, `belief_target_evidence`, and `initial_road_d1_scope` capabilities before corrected belief-level native MCTS or opening-road-only D1.
