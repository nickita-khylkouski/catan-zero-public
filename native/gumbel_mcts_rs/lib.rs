...
pub trait Evaluator {
    ...
    fn evaluate(&mut self, game: &Game, legal_action_indices: &[usize])
    ...
    fn evaluate_many(&mut self, requests: &[(Game, Vec<usize>)])
        -> Result<Vec<(HashMap<usize, f64>, f64, f64)>, String>
    {
        let mut out = Vec::with_capacity(requests.len());
        for (game, legal) in requests {
            out.push(self.evaluate(game, legal)?);
        }
        Ok(out)
    }
}
...
pub struct GumbelMctsEngine {
    ...
    /// Each entry: (node_idx, path_from_root, game, legal_indices)
    pending_leaves: Vec<(usize, Vec<(usize, Option<usize>)>, Game, Vec<usize>)>,
...
impl GumbelMctsEngine {
    ...
    pub fn search<E: Evaluator>(
        ...
        // For each root action, compute the child state and evaluate it.
        // This turns N separate evaluator calls into 1 batched call,
        // which is critical for GPU efficiency (1 big kernel vs N small ones).
    ...
    fn expand_node<E: Evaluator>(&mut self, arena: &mut Arena, node_idx: usize, evaluator: &mut E) -> Result<f64, String> {
        ...
        let (priors, value, uncertainty) = evaluator.evaluate(&game, &legal_indices)?;
    ...
    fn pre_expand_root_children<E: Evaluator>(&mut self, arena: &mut Arena, root_idx: usize, evaluator: &mut E) -> Result<(), String> {
        ...
        let mut batch_requests: Vec<(Game, Vec<usize>)> = Vec::new();
        ...
        for (action_idx, action) in actions.iter().enumerate() {
            ...
            for (child_game, prob) in &outcomes {
                ...
                batch_requests.push((child_game.clone(), legal_indices));
    ...
    fn simulate_deferred<E: Evaluator>(
        ...
        if depth >= self.config.max_depth {
            if !arena.get(node_idx).expanded {
                ...
                self.pending_leaves.push((node_idx, path, game, legal_indices));
    ...
    fn traverse_roll_deferred<E: Evaluator>(
        ...
        mut path: Vec<(usize, Option<usize>)>, evaluator: &mut E,
    ) -> Result<f64, String> {
        ...
        // Record the action stats for backpropagation
        path.push((node_idx, Some(action_idx)));
    ...
    fn flush_pending<E: Evaluator>(&mut self, arena: &mut Arena, evaluator: &mut E) -> Result<(), String> {
        ...
        let requests: Vec<(Game, Vec<usize>)> = self.pending_leaves.iter()
            .map(|(_, _, game, legal)| (game.clone(), legal.clone()))
            .collect();
        ...
        for ((node_idx, path, _, _), (priors, value, uncertainty)) in
            self.pending_leaves.drain(..).zip(results.into_iter())
        {
            ...
            // Fix backpropagation: subtract placeholder (0.0) and add real value
            // Since placeholder was 0.0, we just need to add the real value
            let clamped_value = value.clamp(-1.0, 1.0);
            self.backpropagate(arena, &path, clamped_value);
        }
    ...
    fn enumerate_outcomes<E: Evaluator>(&mut self, arena: &mut Arena, node_idx: usize, action_idx: usize, evaluator: &mut E) -> Result<(), String> {
        ...
        let mut requests: Vec<(Game, Vec<usize>)> = Vec::new();
        for &child_idx in &new_child_indices {
            let child_game = arena.get(child_idx).game.clone();
            let legal = generate_playable_actions(&child_game.state);
            let legal_indices: Vec<usize> = (0..legal.len()).collect();
            requests.push((child_game, legal_indices));
        }
        let results = evaluator.evaluate_many(&requests)?;
    ...
    fn forced_single_action<E: Evaluator>(&mut self, game: &Game, legal: &[Action], root_color: Color, evaluator: &mut E) -> Result<SearchResult, String> {
        ...
        if action.action_type != ActionType::Roll {
            let (_, value, _) = evaluator.evaluate(game, &[0])?;
        ...
        let mut weighted = 0.0;
        for (child_game, prob) in &outcomes {
            let (_, value, _) = evaluator.evaluate(child_game, &[0])?;
            weighted += prob * value;
        }
    ...
    fn raw_policy_root<E: Evaluator>(&mut self, game: &Game, root_color: Color, legal: &[Action], evaluator: &mut E) -> Result<SearchResult, String> {
        let legal_indices: Vec<usize> = (0..legal.len()).collect();
        let (priors, value, _) = evaluator.evaluate(game, &legal_indices)?;
