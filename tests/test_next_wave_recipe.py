...
CONFIG = (
    REPO
    / "configs/experiments/next_wave/coherent_public_n128_adaptive256_forced_value_v3.schema20.json"
)
...
def test_next_wave_typed_generation_config_is_exact_schema_20_recipe() -> None:
    payload = json.loads(CONFIG.read_text())
    assert payload["pipeline"] == GenerateConfig.PIPELINE
    assert payload["schema_version"] == CONFIG_SCHEMA_VERSION == 20
...
def test_canonical_scratch_recipe_has_nontrivial_lr_and_equal_game_value_mass() -> None:
    ...
    assert "value_blocks" in recipe["require_feature_learning_signal_modules"]
    assert "value_head" in recipe["require_feature_learning_signal_modules"]
    assert "value_state_norm" in recipe[
        "require_feature_learning_signal_modules"
    ]
