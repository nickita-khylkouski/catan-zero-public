...
def test_internal_promotion_h2h_must_match_external_tournament_map(
    tmp_path: Path,
) -> None:
    fixture = _fixture(tmp_path)

    def use_historical_random_base_map(source: dict) -> None:
        source["typed_config"]["fields"]["map_kind"] = "BASE"
    ...
    _mutate_evidence_source(
        ...
        kind="internal_h2h",
        role="internal_h2h",
        mutate=use_historical_random_base_map,
    )

    with pytest.raises(promotion.PromotionError, match="must use TOURNAMENT"):
        _execute(fixture, go=False)
