...
def test_confirmed_shard_path_rejects_escape_and_noncanonical_names(
    ...
    with pytest.raises(executor.ExecutorError, match="filename is not canonical"):
        executor._confirmed_worker_shard_path(
            worker,
            filename=filename,
            index=0,
        )
...
def test_confirmed_shard_path_rejects_symlink_alias(tmp_path: Path) -> None:
    ...
    with pytest.raises(executor.ExecutorError, match="escapes or aliases"):
        executor._confirmed_worker_shard_path(
            worker,
            filename=alias.name,
            index=0,
...
def test_confirmed_shard_path_accepts_canonical_regular_file(tmp_path: Path) -> None:
    worker = tmp_path / "worker_000"
    worker.mkdir()
