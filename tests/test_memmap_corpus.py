...
def _minimal_teacher_shard(path: Path, *, seed: int, opponent: bool) -> None:
    """Small schema-valid shard for provenance round-trip tests."""
...
def test_opponent_provenance_survives_mixed_npz_and_memmap_paths(tmp_path):
    """Plain self-play + tagged opponent shards keep one aligned schema.

    This is the regression that previously erased the 80/15/5 mixture identity
    at NPZ -> memmap conversion time.
    """
    ...
    for field in fields:
        np.testing.assert_array_equal(
            np.asarray(memmap[field]), np.asarray(in_memory[field]), err_msg=field
        )

    np.testing.assert_array_equal(
        np.asarray(memmap["opponent_provenance_present"]),
        [False, False, True, True],
    )
    np.testing.assert_array_equal(
        np.asarray(memmap["opponent_version"]), [-1, -1, 7, 7]
    )
    np.testing.assert_array_equal(
        np.asarray(memmap["opponent_tag"]).astype(str),
        ["", "", "recent_history", "recent_history"],
    )
    np.testing.assert_array_equal(
        np.asarray(memmap["training_source_category_verified"]),
    ...
    assert quality["training_source_category_verified_rows"] == 0
    assert teacher_provenance_quality(memmap, chunk_rows=1) == {
        "opponent_provenance_rows": 2,
        "opponent_provenance_fraction": 0.5,
        "opponent_tag_counts": {"recent_history": 2},
        "training_source_category_verified_rows": 0,
        "training_source_category_verified_fraction": 0.0,
        "training_source_category_counts": {},
    }
...
def test_authenticated_selected_game_categories_are_stamped_without_guessing():
    normalized = {
        ...
        "opponent_tag": np.asarray(
            ["", "", "recent_history", "hard_negative"]
        ),
        "opponent_provenance_present": np.asarray(
...
def test_authenticated_source_category_rejects_raw_tag_contradiction():
    normalized = {
        ...
        "opponent_provenance_present": np.asarray([True], dtype=np.bool_),
    }
