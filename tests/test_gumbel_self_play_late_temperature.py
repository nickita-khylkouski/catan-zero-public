...
from catan_zero.rl.gumbel_self_play import GumbelSelfPlayConfig, _temperature_for_decision
...
def test_late_temperature_disabled_by_default_matches_two_stage_schedule() -> None:
    ...
    for decision_index in (0, 44):
        assert _temperature_for_decision(
...
def test_late_temperature_window_extends_past_the_opening_cutoff() -> None:
    ...
    assert _temperature_for_decision(44, config=config, eval_override=False) == config.temperature_high
    ...
    assert _temperature_for_decision(150, config=config, eval_override=False) == config.temperature_low
    assert _temperature_for_decision(599, config=config, eval_override=False) == config.temperature_low
...
def test_late_temperature_cutoff_never_precedes_the_opening_cutoff() -> None:
    ...
    config = _config(late_temperature_move_fraction=0.01, late_temperature=0.3)  # would be < cutoff

    for decision_index in (45, 90, 599):
        assert (
            _temperature_for_decision(decision_index, config=config, eval_override=False)
            == config.temperature_low
        )
