...
def _policy_metrics(
    ...
    target_entropy = -(np.where(target > 0, target * np.log(np.maximum(target, EPS)), 0.0)).sum(axis=1)
    prior_entropy = -(np.where(prior > 0, prior * np.log(np.maximum(prior, EPS)), 0.0)).sum(axis=1)
    kl = np.where(
        target > 0,
...
def load_npz_target_corpus(root: Path) -> tuple[dict[str, np.ndarray], list[dict[str, Any]]]:
    ...
    for path in shards:
        with np.load(path, allow_pickle=False) as handle:
            missing = {"game_seed", "target_policy", "prior_policy", "target_policy_mask"} - set(handle.files)
            if missing:
                raise ValueError(f"{path}: missing required columns {sorted(missing)}")
            shard = {key: np.asarray(handle[key]) for key in COMPACT_FIELDS if key in handle.files}
    ...
    return {key: np.concatenate(parts, axis=0) for key, parts in result.items()}, inventories
...
def _selected_seeds(manifest_path: Path, category: str | None) -> np.ndarray:
    ...
    if not seeds:
        raise ValueError(f"no selected seeds for category={category!r} in {manifest_path}")
...
def analyze_memmap(
    ...
    *,
    ...
    phase_codes = np.memmap(root / "phase.codes.dat", dtype=np.int32, mode="r", shape=(n_rows,))
    phases = list(phase_meta["categories"])
    offsets = np.memmap(root / "row_offsets.dat", dtype=np.int64, mode="r", shape=(n_rows + 1,))
    flat_count = int(offsets[-1])
    target_flat = np.memmap(root / "target_policy.dat", dtype=np.float32, mode="r", shape=(flat_count,))
    prior_flat = np.memmap(root / "prior_policy.dat", dtype=np.float32, mode="r", shape=(flat_count,))
    ...
    for start in range(0, n_rows, chunk_rows):
        ...
        game_counts.update({int(seed): int(count) for seed, count in zip(unique, counts)})
        ...
        full_mask = np.asarray(fixed["used_full_search"][indices], dtype=bool)
        weights = np.asarray(fixed["policy_weight_multiplier"][indices], dtype=np.float32)
        ...
        if active_indices.size:
            ...
            target_terms = np.where(target > 0, target * np.log(np.maximum(target, EPS)), 0.0)
            prior_terms = np.where(prior > 0, prior * np.log(np.maximum(prior, EPS)), 0.0)
            kl_terms = np.where(
                target > 0,
                target * (np.log(np.maximum(target, EPS)) - np.log(np.maximum(prior, EPS))),
                0.0,
            ...
            prior_entropy_parts.append(-np.add.reduceat(prior_terms, starts)[local_active])
            kl_parts.append(np.add.reduceat(kl_terms, starts)[local_active])
            active_phase_parts.append(active_codes)

    derived = {
        "target_entropy": np.concatenate(entropy_parts) if entropy_parts else np.empty(0),
        "prior_entropy": np.concatenate(prior_entropy_parts) if prior_entropy_parts else np.empty(0),
        "kl_target_prior": np.concatenate(kl_parts) if kl_parts else np.empty(0),
        "active_phase_codes": np.concatenate(active_phase_parts) if active_phase_parts else np.empty(0, dtype=np.int32),
...
def analyze_npz(root: Path, *, compact_out: Path | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    game_counts = Counter({int(seed): int(count) for seed, count in zip(unique, counts)})
    forced_mask = np.asarray(data.get("is_forced", np.zeros(len(seeds))), dtype=bool)
    full_mask = np.asarray(data.get("used_full_search", np.zeros(len(seeds))), dtype=bool)
    weights = np.asarray(data.get("policy_weight_multiplier", np.ones(len(seeds))), dtype=np.float32)
    ...
    phases_all = np.asarray(data.get("phase", np.full(len(seeds), "UNKNOWN"))).astype(str)
    phase_categories = sorted(set(phases_all.tolist()))
    phase_code_map = {phase: i for i, phase in enumerate(phase_categories)}
    active_codes = np.asarray([phase_code_map[phase] for phase in phases_all[active_mask]], dtype=np.int32)
    ...
    report = _finalize_report(
        ...
        simulations=np.asarray(data["simulations_used"]) if "simulations_used" in data else None,
    )
    compact_record = None
...
def _finalize_report(
    *,
    ...
    return {
        ...
        "forced": {"rows": int(forced), "fraction": float(forced / rows) if rows else None},
        "full_search": {"rows": int(full), "fraction": float(full / rows) if rows else None},
        "policy_active": {"rows": int(active), "fraction": float(active / rows) if rows else None},
        "phase_distribution": {
            phase: {"rows": int(count), "fraction": float(count / phase_total) if phase_total else None}
        ...
        "simulations_per_row": _summary(simulations) if simulations is not None else None,
    }


def compare_reports(candidate: dict[str, Any], reference: dict[str, Any]) -> dict[str, Any]:
    def delta(path: Iterable[str]) -> dict[str, float | None]:
        ...
        if left is None or right is None:
            return {"candidate": left, "reference": right, "absolute_delta": None, "relative_delta": None}
        absolute = float(left - right)
        relative = float(absolute / abs(right)) if right != 0 else None
        return {"candidate": float(left), "reference": float(right), "absolute_delta": absolute, "relative_delta": relative}
        ...
        if candidate_phase is not None and reference_phase is not None:
            ...
            for metric in ("target_entropy", "prior_entropy", "kl_target_prior"):
                ...
                result["policy_targets_by_phase"][phase][metric] = {
                    ...
                    "relative_delta": float(absolute / abs(right_mean)) if right_mean else None,
                }
...
def main() -> None:
    ...
    candidate, candidate_provenance = analyze_npz(args.candidate_npz_root, compact_out=args.compact_out)
    ...
    payload = {
        ...
        "provenance": {"candidate": candidate_provenance, "reference": reference_provenance},
    }
