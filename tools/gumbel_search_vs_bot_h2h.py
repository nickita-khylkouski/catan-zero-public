...
MAP_KIND = "TOURNAMENT"  # only map engine_equivalence guarantees Rust<->Python parity for.
...
def play_one_h2h_game(
    *,
    ...
    equiv_config = EquivalenceConfig(
        colors=("RED", "BLUE"), map_kind=MAP_KIND, vps_to_win=10, discard_limit=7,
        friendly_robber=False, max_steps=max(2000, int(max_decisions) * 6),
    )
    rust_game, python_game, seated_colors = build_paired_games(int(game_seed), equiv_config)

    candidate_mcts = GumbelChanceMCTS(
        GumbelChanceMCTSConfig(
            colors=seated_colors, map_kind=MAP_KIND, seed=int(game_seed), **search_config_kwargs
        ),
        candidate_evaluator,
    )
    bot_by_color = {
    ...
    while decision_index < int(max_decisions):
        ...
        if role == "candidate":
            ...
            py_action = bot.decide(python_game.copy(), tuple(python_game.playable_actions))
            key = canonical_python_action_key(py_action)
            matches = [i for i, raw in enumerate(raw_actions) if canonical_rust_action_key(raw) == key]
            if not matches:
                divergence_detail = f"bot action {py_action!r} has no Rust-side equivalent"
                break
            pos = matches[0]
        ...
        try:
            if is_chance_action(chosen_raw):
                rust_game, _outcome = apply_chance_step(
                    rust_game, python_game, chosen_raw, symbols=symbols, harness_rng=chance_rng
                )
            else:
                rust_game.execute_action_index(selected_id, list(seated_colors), MAP_KIND)
        ...
        mismatches = diff_state_views(rust_state_view(rust_game), python_state_view(python_game, symbols))
        if mismatches:
            divergence_detail = (
    ...
    candidate_won = (winner == candidate_color) if (terminal and not engine_divergence) else None
...
def _write_worker_progress(progress_dir: str, worker_index: int, games_done: int, wins: int,
    ...
    try:
        with open(tmp, "w") as fh:
            json.dump({"worker_index": worker_index, "games_done": games_done,
                       "candidate_wins": wins, "engine_divergences": divergences}, fh)
        _os.replace(tmp, p)
    except OSError:
        pass
    ...
    try:
        for pair in worker_args["pairs"]:
            ...
            _write_worker_progress(worker_args.get("progress_dir", ""),
                                   int(worker_args["worker_index"]), len(games), _wins, _divs)
    finally:
...
def main() -> None:
    ...
    parser.add_argument("--correct-rust-chance-spectra", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--lazy-interior-chance", action=argparse.BooleanOptionalAction, default=False)
    ...
    if len(worker_args) <= 1:
        ...
        with ctx.Pool(processes=len(worker_args)) as pool:
            for done, result in enumerate(pool.imap_unordered(_worker_entry, worker_args), start=1):
                ...
                _d = sum(1 for r in results for gm in r.get("games", ()) if gm.get("engine_divergence"))
                print(json.dumps({"progress": "worker_done", "workers_done": done,
                                  "workers_total": len(worker_args), "games_so_far": _g,
                                  "candidate_wins_so_far": _w, "engine_divergences_so_far": _d,
    ...
    summary = _build_summary(
        args, all_games=all_games, outcomes=outcomes, truncated_count=truncated_count,
        divergence_count=divergence_count, pairs=pairs, elapsed=elapsed, workers=workers,
        threads_per_worker=threads_per_worker, errors=errors,
    )
...
def _build_summary(
    args: Any, *, all_games: list[dict[str, Any]], outcomes: list[bool], truncated_count: int,
    divergence_count: int, pairs: list[Any], elapsed: float, workers: int,
    threads_per_worker: int, errors: list[Any],
) -> dict[str, Any]:
    ...
    win_rate = (sum(1 for outcome in outcomes if outcome) / len(outcomes)) if outcomes else None
