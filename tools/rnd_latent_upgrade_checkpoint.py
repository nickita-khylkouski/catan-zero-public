#!/usr/bin/env python3
"""Warm-grow a K=0 RRT checkpoint with fixed-K latent deliberation.

This R&D-only transaction copies every source model tensor byte-for-byte and
adds only the fixed-K deliberation tensors.  The deliberation fusion is zero at
initialisation, and the transaction verifies exact output equality on a
deterministic schema-shaped public batch before publishing the checkpoint.  It
does not train, evaluate playing strength, or promote a model.
"""
...
from catan_zero.rl.action_features import (  # noqa: E402
...
SCHEMA_VERSION = "catan-zero-latent-checkpoint-upgrade/v1"
...
def _public_synthetic_batch(config: EntityGraphConfig) -> dict[str, torch.Tensor]:
    """Return a deterministic public-schema-shaped RRT batch on CPU."""
    ...
    batch["hex_vertex_ids"] = torch.full(
        (batch_size, 19, 6), -1, dtype=torch.long
    )
    batch["hex_edge_ids"] = torch.full(
        (batch_size, 19, 6), -1, dtype=torch.long
    )
    batch["edge_vertex_ids"] = torch.full(
        (batch_size, 72, 2), -1, dtype=torch.long
    )
...
def _publish_no_overwrite(payload: object, output: Path) -> None:
    """Atomically publish a torch payload without ever replacing a path."""
    ...
    try:
        ...
        try:
            ...
            # EEXIST; unlike os.replace it cannot clobber a concurrent writer.
            os.link(temporary, output)
        except FileExistsError as error:
            raise FileExistsError(
                f"latent upgrade refuses to overwrite {output}"
            ) from error
...
def upgrade_checkpoint(
    ...
    *,
    ...
    if source_path == output_path:
        raise ValueError("latent upgrade output must differ from its source checkpoint")
    if output_path.exists():
        raise FileExistsError(f"latent upgrade refuses to overwrite {output_path}")
    if int(steps) < 1:
    ...
    if str(getattr(config, "state_trunk", "transformer") or "transformer") != "rrt":
        raise ValueError("latent upgrade source must use state_trunk='rrt'")
    if int(getattr(config, "latent_deliberation_steps", 0) or 0) != 0:
        raise ValueError("latent upgrade source must be a K=0 checkpoint")
    source_deliberation = [
        str(name)
    ...
    upgraded_policy = EntityGraphPolicy(
        upgraded_config, static_array, seed=0, device="cpu"
    )
    ...
    if invalid_missing or unexpected:
        raise RuntimeError(
            "latent upgrade tensor mismatch: "
            f"invalid_missing={invalid_missing[:8]} unexpected={unexpected[:8]}"
        )
    if not missing:
        raise RuntimeError("latent upgrade added no deliberation tensors")
    ...
    for name, tensor in payload["model"].items():
        if not torch.equal(upgraded_state[name].cpu(), tensor.cpu()):
            raise RuntimeError(f"shared tensor changed during latent upgrade: {name}")
    added = sorted(set(upgraded_state) - set(payload["model"]))
    if added != sorted(missing):
        raise RuntimeError(
            "latent upgrade additions differ from missing tensors: "
            f"added={added[:8]} missing={sorted(missing)[:8]}"
        )
    if any(not name.startswith(_DELIBERATION_PREFIXES) for name in added):
        raise RuntimeError(f"latent upgrade added a non-deliberation tensor: {added[:8]}")
    ...
    for name, source_tensor in source_outputs.items():
        output_tensor = upgraded_outputs[name]
    ...
    upgraded_payload["latent_deliberation_upgrade"] = provenance
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.report and Path(args.report).exists():
        raise FileExistsError(f"latent upgrade refuses to overwrite {args.report}")
