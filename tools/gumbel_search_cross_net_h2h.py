...
def _game_search_seed(*, game_seed: int, seat_color: str) -> int:
    ...
    Search objects are reused to avoid repeatedly loading models, but their RNG
    state must not be inherited from earlier variable-length games. Seat-keying
    also preserves the paired color-swap contract: RED receives the same search
    stream in both orientations regardless of which checkpoint occupies it.
    """
