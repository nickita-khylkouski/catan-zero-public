...
def load_manifest(path: Path) -> dict[str, Any]:
    ...
    for raw in value["hosts"]:
        ...
        accelerator = str(raw.get("accelerator", "H100"))
        repo_commit = str(raw.get("repo_commit", ""))
        if not accelerator or any(char in accelerator for char in "\n\r\0"):
            raise FleetError(f"invalid accelerator for {alias}")
    ...
    if actual != EXPECTED_SHAPES:
        raise FleetError(
            "canonical fleet must be exactly six 4xH100 plus h100-8a/h100-8b: "
...
"""
    ...
    def inspect(host: dict[str, Any]) -> dict[str, Any]:
        ...
        topology_ok = (
            ...
            and host["accelerator"].lower() in fields.get("gpu_names", "").lower()
        )
...
test "$(nvidia-smi --query-gpu=name --format=csv,noheader | grep -ci {shlex.quote(host["accelerator"])})" -eq {host["gpu_count"]}
"""
...
def _launch_command(
    ...
    pid_path = shlex.quote(job_dir + "/.pid")
    return "\n".join(
        [
            f"mkdir -p {shlex.quote(job_dir)}",
            f"test ! -e {receipt_path} || "
            f'test "$(base64 -w0 {receipt_path})" = {shlex.quote(encoded)}',
            f"if [ -f {shlex.quote(job_dir + '/.done')} ]; then "
            f"echo {shlex.quote(row['job_id'] + ':done')};",
            f'elif [ -s {pid_path} ] && kill -0 "$(cat {pid_path})" 2>/dev/null; '
            f"then echo {shlex.quote(row['job_id'] + ':active')};",
            "else",
            _gpu_idle_check(row),
            f"printf %s {shlex.quote(encoded)} | base64 -d > "
            f"{shlex.quote(job_dir + '/receipt.tmp')}",
            f"chmod 0444 {shlex.quote(job_dir + '/receipt.tmp')}; "
            f"mv -f {shlex.quote(job_dir + '/receipt.tmp')} {receipt_path}",
...
def status(
    ...
    *,
    ...
    def inspect(row: dict[str, Any]) -> dict[str, Any]:
        ...
        command = (
            f'd={directory}; if [ -f "$d/.done" ]; then s=DONE; '
            f'elif [ -f "$d/.failed" ]; then s=FAILED; '
            f'elif [ -s "$d/.pid" ] && kill -0 "$(cat "$d/.pid")" '
            f'2>/dev/null; then s=RUNNING; elif [ -e "$d/receipt.json" ]; '
            f"then s=LOST; else s=NOT_SUBMITTED; fi; printf '%s' \"$s\""
        )
