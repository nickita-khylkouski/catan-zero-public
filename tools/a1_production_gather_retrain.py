#!/usr/bin/env python3
"""Replay, but never newly execute, the historical full-dose gather retrain.

The v1 operator remains verifiable/finalizable for receipts that already
exist.  Its 4,194,304-row dose was superseded by the selected 524,288-row
learner measure, so it can no longer create a new one-shot claim or launch.
"""
...
LEGACY_MANIFEST_SCHEMA = "a1-production-target-gather-retrain-v1"
MANIFEST_SCHEMA = "a1-production-target-gather-retrain-v2-obsolete-dose"
...
HISTORICAL_ONLY_REASON = (
    "production target-gather v1 uses the superseded 4,194,304-row dose; "
    "commission a typed selected-dose promotion operator before launching"
)
BOUND_SOURCE_FILES = (
...
def prepare(
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "obsolete_reason": HISTORICAL_ONLY_REASON,
...
def verify(manifest_path: Path) -> dict[str, Any]:
    ...
    authority_matches_schema = (
        ...
        and "historical_only" not in manifest
        and "obsolete_reason" not in manifest
        if legacy
        else manifest.get("diagnostic_only") is True
        and manifest.get("production_eligible") is False
        and manifest.get("launch_authorized") is False
        and manifest.get("historical_only") is True
        and manifest.get("obsolete_reason") == HISTORICAL_ONLY_REASON
        if schema == MANIFEST_SCHEMA
        else False
    )
    if stated != base._digest(unhashed) or not authority_matches_schema:  # noqa: SLF001
    ...
    expected_source_files = LEGACY_BOUND_SOURCE_FILES if legacy else BOUND_SOURCE_FILES
    if set(repo_binding.get("files", {})) != set(expected_source_files):
        raise GatherRetrainError("repository source binding is incomplete")
...
def execute(
    ...
    *,
    ...
    # Verification and finalization remain available for immutable historical
    # receipts.  Crucially, refusal happens before idle probes, output-root
    # creation, durable claims, or systemd submission.
    raise GatherRetrainError(HISTORICAL_ONLY_REASON)
...
def finalize(
    ...
    *,
    ...
    if verified["manifest"]["schema_version"] != LEGACY_MANIFEST_SCHEMA:
        raise GatherRetrainError(HISTORICAL_ONLY_REASON)
    root = verified["output_root"]
